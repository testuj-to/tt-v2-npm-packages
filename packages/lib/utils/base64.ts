...
const encodeProcedures = {
    buffer: (escapedValue: string) =>
        `return Buffer.from('${escapedValue}').toString('base64');`,
    btoa: (escapedValue: string) =>
        `return btoa('${escapedValue}');`,
};

const decodeProcedures = {
    buffer: (escapedValue: string) =>
        `return Buffer.from('${escapedValue}', 'base64').toString('utf8');`,
    atob: (escapedValue: string) =>
        `return atob('${escapedValue}');`,
};
